...
<body>
    ...
    <div class="wrapper">
        ...
        <div class="main_body">
            ...
            <div class="container">
                <h2><span class="icon"><i class="fas fa-calendar-week"></i></span>
                    <span class="list">Calendario de citas</span>
                </h2>
                <br>
                <div class="formx4">
    <?php
        ...
        foreach ($meses as $mes => $nombre_mes) {
            ...
            $sql = "SELECT citas.*, servicios.Nombre_servicio, clientes.Nombre, clientes.Apellido_paterno, clientes.Apellido_materno, mascotas.Alias, asistencia.S_N
                FROM citas 
                JOIN servicios ON citas.Servicio = servicios.id_servicio 
                JOIN asistencia ON citas.Asistencia = asistencia.id_asistencia
                JOIN clientes ON citas.Cliente = clientes.id_cliente
                JOIN mascotas ON citas.Paciente = mascotas.id_mascota
                WHERE citas.Fecha BETWEEN '$primer_dia' AND '$ultimo_dia' 
    ...
    <!-- Modal HTML -->
    <div id="modal"
        style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 80%; max-width: 600px; background: white; border: 1px solid #ccc; box-shadow: 0 4px 8px rgba(0,0,0,0.2); padding: 20px; z-index: 1000; overflow-y: auto;">
        <h2>Detalles de la cita</h2>
        <div id="modal-content"></div>
        <button class="button-login1" onclick="closeModal()" style="margin-top: 20px;">Cerrar</button>
    </div>
        ...
        function showModal(citasJson) {
            ...
            citas.forEach(function (cita) {
                modalContent.innerHTML += '<p><strong>Folio:</strong> ' + cita.Folio + '<br>' +
                    '<strong>Cliente:</strong> ' + cita.Nombre + ' ' + cita.Apellido_paterno + ' ' + cita.Apellido_materno + '<br>' +
                    '<strong>Paciente:</strong> ' + cita.Alias + '<br>' +
                    '<strong>Hora:</strong> ' + cita.Hora + '<br>' +
                    '<strong>Servicio:</strong> ' + cita.Nombre_servicio + '<br>' +
                    '<strong>Nota:</strong> ' + cita.Nota_motivo + '<br>' +
                    '<strong>Asistencia de la cita:</strong> ' + cita.S_N + '</p>' +
